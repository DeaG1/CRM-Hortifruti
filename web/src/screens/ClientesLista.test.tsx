...
describe('ClientesLista — os quatro estados', () => {
  it('carregando: mostra indicador enquanto a chamada esta pendente', () => {
    ...
    render(<ClientesLista papel="admin" onAbrir={() => {}} />)
...
describe('ClientesLista — sessao expirada (401)', () => {
  it('chama onSessaoExpirada em vez de mostrar a mensagem de erro generica', async () => {
    ...
    render(<ClientesLista papel="admin" onAbrir={() => {}} onSessaoExpirada={onSessaoExpirada} />)
...
describe('ClientesLista — vendas reais (GET /api/saidas)', () => {
  ...
  it('clicar numa linha chama onAbrir com o id do cliente', async () => {
    ...
    render(<ClientesLista papel="admin" onAbrir={onAbrir} />)
...
describe('ClientesLista — periodo global', () => {
  it('as metricas so contam as vendas do periodo escolhido', async () => {
    ...
    render(<ClientesLista papel="admin" onAbrir={() => {}} periodo="2026-06" />)
    const linha = await screen.findByText('Mercado A')
  ...
  it('o CADASTRO nao some num periodo sem venda nenhuma', async () => {
    ...
    render(<ClientesLista papel="admin" onAbrir={() => {}} periodo="2026-01" />)
...
/**
 * O colaborador passou a ver e editar o CADASTRO de clientes, e a NAO ver os
 * numeros da carteira (faturado, ticket por entrega, participacao,
 * inadimplencia, health score) — ver `podeVerMetricasDeCadastro` em telas.ts.
 *
 * RESSALVA HONESTA, registrada aqui porque e o ponto que mais confunde:
 * estes numeros saem de `GET /api/saidas`, que responde para o colaborador
 * (Saidas e tela dele). Esconder as colunas NAO e protecao — ele consegue
 * recalcular por fora, e o dono aceitou isso. O que a tela nao faz e ENTREGAR
 * o agregado pronto, e por isso ela tambem nao busca.
 */
describe('ClientesLista — colaborador ve o cadastro, nao os numeros da carteira', () => {
  const COLUNAS_DE_METRICA = ['TICKET/MÊS', '/ENTREGA', '% FAT', 'INADIMP.', 'SAÚDE']

  it('nao dispara GET /api/saidas — a tela nao pede o que nao vai mostrar', async () => {
    mockRotas([cliente()], [saida()])
    render(<ClientesLista papel="colaborador" onAbrir={() => {}} />)
    await screen.findByText('Mercado A')

    const rotas = mockGet.mock.calls.map(c => c[0] as string)
    expect(rotas).toContain('/api/clientes')
    expect(rotas).not.toContain('/api/saidas')
  })

  it('nao mostra nenhuma das cinco colunas de metrica', async () => {
    mockRotas([cliente()], [saida({ valor: 5000 })])
    render(<ClientesLista papel="colaborador" onAbrir={() => {}} />)
    await screen.findByText('Mercado A')
    ...
    // A nota do recorte fala dos "números da carteira" que nao estao ali.
    expect(screen.queryByText(/números da carteira/)).not.toBeInTheDocument()
  })

  it('mostra o cadastro: estabelecimento, responsavel, rota e frequencia', async () => {
    mockRotas([cliente({ nome: 'Mercado A', resp: 'Sonia', rota: 'Norte', freq: 'Semanal' })])
    render(<ClientesLista papel="colaborador" onAbrir={() => {}} />)
  ...
  it('a linha continua abrindo a ficha (e onde ele edita)', async () => {
    const onAbrir = vi.fn()
    mockRotas([cliente()])
    render(<ClientesLista papel="colaborador" onAbrir={onAbrir} />)
    fireEvent.click(await screen.findByText('Mercado A'))
    expect(onAbrir).toHaveBeenCalledWith('1')
  })

  it('admin continua vendo as cinco colunas e buscando as vendas', async () => {
    mockRotas([cliente()], [saida({ valor: 5000 })])
    render(<ClientesLista papel="admin" onAbrir={() => {}} />)
    await screen.findByText('Mercado A')
